...
public class MoveMotherNatureSceneController extends AbstractSceneController {
    ...
    public void configureIsland(TilePane island, int id)
    {
        ...
        island.setOnDragDropped((DragEvent event)
            ...
            System.out.println(parseInt(event.getDragboard().getString()));
            System.out.println(id - parseInt(event.getDragboard().getString()));
            client.send(new MoveMotherNatureMessage(id - (parseInt(event.getDragboard().getString()) + 1)));
            event.consume();
        });
    }

    public void configureMn(int islandIndex, int mNIndex)
    {
        //detected
        //drop

        islandStudents.get(islandIndex).getChildren().get(mNIndex).setOnDragDetected((javafx.scene.input.MouseEvent event) ->
        {
            Dragboard db = islandStudents.get(islandIndex).getChildren().get(mNIndex).startDragAndDrop(TransferMode.MOVE);
            ClipboardContent content = new ClipboardContent();

            ImageView mnImage = (ImageView) islandStudents.get(islandIndex).getChildren().get(mNIndex);

            content.putImage(mnImage.getImage());
            content.putString(valueOf(islandIndex));
            //content.putString(color);
            db.setContent(content);
            islandStudents.get(islandIndex).getChildren().remove(mNIndex);
